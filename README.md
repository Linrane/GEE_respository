...
本代码为文章引用，原文地址为：https://mp.weixin.qq.com/s/9xodebPWHlXYCAidWX-CDg

整体实现思路：

```
1.配置基础参数：指定 GEE 影像 / 影像集 ID、本地输出根目录、研究区（可选）、目标坐标系、下载分辨率、输出文件名

2.定义 GEE 初始化函数：尝试直接初始化，失败则触发认证流程，指定项目 ID 完成初始化

3.定义研究区获取函数：支持空值（下载全范围）、资产 ID 字符串、几何对象三种输入形式，返回对应几何范围

4.定义核心下载函数：

  创建输出目录，加载指定 ID 的 GEE 数据（优先按影像集加载，单影像 / 多影像分别处理；失败则按单影像加载）

  获取影像波段、日期、边界等信息，按指定研究区裁剪影像（无则使用全范围）

  检查输出文件是否存在，支持覆盖确认，调用 geemap 下载函数将影像按指定分辨率 / 坐标系下载到本地

  输出下载耗时、文件大小等统计信息，捕获异常并返回下载状态

5.定义数据检查函数：分别尝试以影像集 / 单影像加载数据，输出影像数量、波段信息等结构信息

6.定义主函数：解析命令行参数，支持inspect（检查数据）、help（显示帮助）、默认（执行下载）三种模式，执行对应功能并反馈任务结果
```

# 21.NDVI时序Whittaker平滑滤波的使用和理解

本代码为文章引用，原文地址为：https://mp.weixin.qq.com/s/dMvtYTs69rXWLnmSDkaF7Q

代码运行：https://code.earthengine.google.com/f01afc6dbb40ea1831a03460ca0739b4

整体实现思路：

```
1.将指定点要素添加红色图层到地图，将视图居中到该点（缩放级别 7）

2.定义位运算提取函数：按起始 / 结束位提取 QA 波段的特定比特位，转换为标记值

3.定义差分矩阵计算函数：递归生成指定阶数的差分矩阵，用于后续平滑计算

4.定义数组影像解包函数：将数组影像拆分为多波段影像集合，按指定 ID 和波段名命名

5.定义对数比逆变换函数：将回归结果的对数比转换回百分比单位

6.定义核心 Whittaker 平滑函数：
  设置默认参数（非组成数据、平滑系数 λ=2），将影像集转为浮点型

  构建单位矩阵和 3 阶差分矩阵，计算平滑矩阵（hat matrix）

  若为组成数据，先对影像做对数比变换；将影像集转为数组影像，通过矩阵求解实现 Whittaker 平滑

  将平滑后的数组影像解包为影像集，对组成数据做逆变换，重命名波段（添加_fitted 后缀）

  合并原始数据和平滑数据，恢复影像属性和时间戳，计算均方根误差（RMSE）

7.加载 2019 年 VIIRS VNP13A1 NDVI 影像集，用均值填充掩膜像素以消除缺失值

8.调用 Whittaker 平滑函数处理 NDVI 影像集，筛选 2019 年 4 月 20-25 日的影像

9.将原始 NDVI 和平滑后的 NDVI_fitted 分别添加到地图可视化（黑 - 绿配色，值范围 0-6000）

10.创建时序折线图，展示研究点处原始 NDVI 与平滑 NDVI 的均值变化趋势，配置标题、线宽、点大小等样式
```

# 22.GEE算力优化分块处理

本代码为文章引用，原文地址为：https://mp.weixin.qq.com/s/WTQp674kaMYmug89_S8nYQ

代码运行：https://code.earthengine.google.com/c16e7c5ac1821fa9c7672b36f24fb3c0

整体实现思路：

```
1.设定输入多边形为 AOI3，横向分块数为 4、纵向分块数为 3，调用自定义分块函数生成对应的矩形格网。

2.将原始多边形以蓝色样式、分块生成的矩形以红色样式添加到地图可视化，同时将地图视图居中到原始多边形，缩放级别设为 4。

3.定义核心分块函数shpRectBlock，首先校验输入数据类型，仅允许 Geometry、Feature、FeatureCollection 三种类型，非指定类型则抛出错误提示。

4.统一处理输入数据，将 Feature 或 FeatureCollection 类型的输入转换为 Geometry 几何对象，仅保留几何边界信息。

5.计算输入矢量的外接矩形边界，提取边界的 xmin、ymin、xmax、ymax 四个经纬度极值。

6.根据设定的横向、纵向分块数，计算每个矩形分块的经纬度步长，即单个矩形的宽度和高度。

7.通过嵌套遍历行列序号，逐一生成对应的矩形几何对象，默认保留完整矩形（可开启与原始矢量的交集裁剪功能）。

8.将生成的矩形列表转换为 FeatureCollection 要素集合，筛选出与原始输入矢量相交的矩形块，返回最终的分块结果。
```
